    ...
    public boolean validExpressionTree(List<E> op){
        for (Position position : positions()){
            if (isInternal(position)){
                if (!op.contains(position.getElement())){
                    return false;
                }
            }

            if (isExternal(position)){
                if (sibling(position) == null){
                    return false;
                }
                //Chequeo si cada op tiene de hijos a numeros
                try{
                    Double.parseDouble((String) position.getElement());
